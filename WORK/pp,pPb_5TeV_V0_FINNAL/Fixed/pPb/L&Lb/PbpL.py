...
Xleft, DeltaXtime2, Y1, DeltaY1, Y2, DeltaY2, Y3, DeltaY3, Y4, DeltaY4 = np.loadtxt(DATA, unpack=True)
#Xleft0, DeltaXtime20, Y10, DeltaY10, Y20, DeltaY20, Y30, DeltaY30, Y40, DeltaY40 = np.loadtxt(DATA, unpack=True)
XleftReper, DeltaXtime2Reper, Y1Reper, DeltaY1Reper, Y2Reper, DeltaY2Reper, Y3Reper, DeltaY3Reper, Y4Reper, DeltaY4Reper = np.loadtxt(DATA, unpack=True)
...
while i <= len(x)-1:
	...
	while j <= len(x)-1:
		if x[i] == XleftReper[j]:
			ex[i] = DeltaXtime2Reper[j]
			
			y1[i] = Y1Reper[j]
			ey1[i] =  DeltaY1Reper[j]
			
			y2[i] = Y2Reper[j]
			ey2[i] =  DeltaY2Reper[j]
			
			y3[i] = Y3Reper[j]
			ey3[i] =  DeltaY3Reper[j]
			
			y4[i] = Y4Reper[j]
			ey4[i] =  DeltaY4Reper[j]
...
minuit2.DefineParameter(0, 'Area', 500, 1e-1, 0., 0.)
...
minuit3.DefineParameter(0, 'Area', 500, 1e-1, 0., 0.)
...
minuit4.DefineParameter(0, 'Area', 840, 1e-1, 0., 0.)
minuit4.DefineParameter(1, 'Temper', 0.3, 1e-4, 0., 0.)
minuit4.DefineParameter(2, 'Q', 1.15, 1e-3, 0., 0.)
...
Plot1.SetTitle("#Lambda^{0} + #bar{#Lambda^{0}} Pbp #sqrt{s_{NN}}= 5.02 TeV")
...
Legend.AddEntry(Plot1,'-4.5 < y < -4.0, T_{init}= 0.946 GeV', 'ep')
Legend.AddEntry(Plot2,'-4.0 < y < -3.5, T_{init}= 1.001 GeV', 'ep')
Legend.AddEntry(Plot3,'-3.5 < y < -3.0, T_{init}= 1.027 GeV', 'ep')
#Legend.AddEntry(Plot4,'-3.0 < y < -2.5', 'ep')
