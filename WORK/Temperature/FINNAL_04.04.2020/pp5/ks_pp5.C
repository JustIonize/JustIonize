...

{
    ...
    f = new TF1("f", "[1]*((x)^2)*exp(-sqrt((x)^2 + pow(m,2))*[0])",0, 6000000000);
    f->SetParameter(2, 1);
    f->SetParameter(1, 1);
    f->SetParameter(0, 4);
    G1->Fit(f,"N");
    ...
    h = new TF1("h", "[1]*x*sqrt((x)^2+m^2)*exp(-sqrt((x)^2 + pow(m,2))*[0])",0, 6000000000);
    h->SetParameter(2, 1);
    h->SetParameter(1, 1);
    h->SetParameter(0, 4);
    G2->Fit(h,"N");
    ...
    gPad->SetLogy(1);   
    G3 = new TGraph("ks_pp5.txt");
    G3->SetTitle("mod. Maxwell-Boltzman 2; pT GeV ; d(sigma)/d(pT) pb^(-1) ");      
    j = new TF1("j", "[1]*x*exp(-sqrt((x)^2 + pow(m,2))*[0])",0, 6000000000);
    G3->Fit(j,"N");
    G3->Draw("APSAME");
    g3->Draw("P");
    j->SetLineWidth(5);
    ...
    k = new TF1("k", "[2]*2*pi*x*pow((1-(sqrt(pow(0.497648,2)+(x)^2)  - 0.497648 )*[0]*[1])  ,  (1/[1])  )",0, 6000000000);
    k ->SetParameter(1, -1);
    k ->SetParameter(0,4);
    k ->SetParameter(2,1);
    G4->Fit(k,"N");
