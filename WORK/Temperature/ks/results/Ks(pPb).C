...

{
    ...
    f = new TF1("f", "[1]*((x)^2)*exp(-sqrt((x)^2 + pow(m,2))*[0])",0, 10);
    G1->Fit(f,"N");
    G1->Draw("APSAME");
    g1->Draw("P");
    f->SetLineWidth(5);
    ...
    h = new TF1("h", "[1]*x*sqrt((x)^2+m^2)*exp(-sqrt((x)^2 + pow(m,2))*[0])",0, 10);
    G2->Fit(h,"N");
    G2->Draw("APSAME");
    g2->Draw("P");
    h->SetLineWidth(5);
    ...
    j = new TF1("j", "[1]*x*exp(-sqrt((x)^2 + pow(m,2))*[0])",0, 10);
    G3->Fit(j,"N");
    G3->Draw("APSAME");
    g3->Draw("P");
    j->SetLineWidth(5);
    ...
    k = new TF1("k", "[2]*2*pi*x*pow((1-(sqrt(pow(0.497648,2)+(x)^2)  - 0.497648 )*[0]*[1])  ,  (1/[1])  )",0, 10);
    k ->SetParameter(1, -1);
    k ->SetParameter(0,4);
    k ->SetParameter(2,1);
    G4->Fit(k,"N");
    ...
    c1->cd(5);
    g5 = new TGraphErrors("NEWks(pPb).txt");
    g5->SetMarkerStyle(21);
    gPad->SetLogy(1);    
    G5 = new TGraph("NEWks(pPb).txt");
    G5->SetTitle("Maxwell-Boltzman+; pT GeV ; d(sigma)/d(pT) pb^(-1) ");
    t = new TF1("t", " [1]*x*pow((1+(x*[0])*(x*[0])),(-[2]))",0, 10);
    t->SetParameter(2, 1);
    t->SetParameter(1, 1);
    G5->Fit(t,"N");
    G5->Draw("APSAME");
    ...
    //latex.DrawLatex( 3.3, -10, "p_{T} (GeV)");

    TLatex latexY;
    latexY.SetTextSize(0.04);
    latexY.SetTextAlign(10);
    latexY.SetTextAngle(90);
    //latexY.DrawLatex(-0.2,0.2,"d#sigma/dp_{T} (pb^{-1})");

    TLatex latexD;
    latexD.SetTextSize(0.04);
    latexD.SetTextAlign(40);
    latexD.SetTextAngle(0);
    latexD.DrawLatex(1.5,9,"K_{s}^{0} LHCb p-Pb #sqrt{s_{NN}}= 5.02 TeV");

 //   TMathText lin;
 //   lin.SetTextAlign(50);
 //   lin.SetTextSize(0.03);
 //   lin.DrawMathText(3.3,0.6,"#frac{sin(x)}{x}");
