...
minuit4.DefineParameter(0, 'Area', 500, 1e-1, 0., 0.)
...
f = open("PbpK2FixedRESULTS.txt", "w")
...
f = open("PbpK2FixedRESULTS.txt", "a")
...
Plot1.SetTitle("K_{s}^{0} Pbp #sqrt{s_{NN}}= 5.02 TeV")
...
Legend.AddEntry(Plot1,'-4.5 < y < -4.0, T_{init}= 0.751 GeV', 'ep')
Legend.AddEntry(Plot2,'-4.0 < y < -3.5, T_{init}= 0.825 GeV', 'ep')
Legend.AddEntry(Plot3,'-3.5 < y < -3.0, T_{init}= 0.792 GeV', 'ep')
Legend.AddEntry(Plot4,'-3.0 < y < -2.5, T_{init}= 0.830 GeV', 'ep')
